        ...
        <div class="page-wrapper">
            ...
            <div class="page-breadcrumb">
                <div class="row">
                    ...
                    <form method="GET" action="{{ route('admin.listbarang') }}">
                        <div class="d-flex justify-content-end pe-4"> <!-- Tambah padding-end untuk geser ke kanan -->
                            <div class="customize-input">
                                <select name="kategori"
                                    class="custom-select form-control bg-white border-0 custom-shadow custom-radius px-3 py-2"
                                    style="width: 240px;" onchange="this.form.submit()">
                                    <option value="Semua Barang"
                                        {{ request('kategori') == 'Semua Barang' ? 'selected' : '' }}>
                                        Semua Barang
                                    </option>
                                    <option value="Elektronik" {{ request('kategori') == 'Elektronik' ? 'selected' : '' }}>
                                        Elektronik
                                    </option>
                                    <option value="Perkakas" {{ request('kategori') == 'Perkakas' ? 'selected' : '' }}>
                                        Perkakas
                                    </option>
                                    <option value="Komponen" {{ request('kategori') == 'Komponen' ? 'selected' : '' }}>
                                        Komponen
                                    </option>
                                ...
                                <table id="zero_config" class="table table-striped table-bordered no-wrap">
                                    ...
                                    <tbody>
                                        @foreach ($databarang as $item)
                                            <tr>
                                                <td>
                                                    @if (trim($item->status) === 'In Progress')
                                                        <span class="badge badge-light-warning">In Progress</span>
                                                    @elseif (trim($item->status) === 'Closed')
                                                        <span class="badge badge-light-danger">Closed</span>
                                                    @elseif (trim($item->status) === 'Opened')
                                                        <span class="badge badge-light-success">Opened</span>
                                                    @else
                                                        <span
                                                            class="badge badge-light-secondary">{{ $item->status ?? 'Unknown' }}</span>
                                                    @endif
                                                </td>
